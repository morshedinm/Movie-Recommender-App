...
def get_user_rating(movies):
    ...
    """
    user_ratings = pd.read_csv("Dataset/User/Users Ratings.csv", dtype={'Year':str})
    movie_id_list = []
    #print(beta_ratings)
...
def cleaning_data(dataset):
    ...
    """
    dataset.dropna(axis=0, thresh=11, inplace=True)
    dataset.dropna(axis=1, thresh=51, inplace=True)
    dataset.fillna(0, inplace=True)
    return dataset


def cluster_movies(movies, n_clusters=4):
    """
    this funtion clusters movies based on their genre. it uses k-means algorithm.
    :param movies: gets the movies data frame whose columns are movieId, title, and genres.
    :param n_clusters: number of clusters that the users want for their clustering
    ...
    # note that we can give these results to a Database for lower the calculation time for the next runs.
...
def cluster_users(dataset, n_clusters):
    ...
    :param n_clusters: this variable is number of clusters
...
def genre_filter(movie_name, dataset, movies):
    ...
    :param movie_name: the movie that we want to recommend based on it
    :param dataset: it is the final dataset, its rows are movies and the columns are ratings
    :param movies: the movies table which is used for finding the movie ID of the movie that this function got.
    ...
    if len(movie_id) > 0:  # examines if the movie id is available or not
        movie_id = movie_id[0]
        indexes, clusters = cluster_movies(movies)  # uses cluster_movies function to cluster the movies and gives
        # the results.
...
def get_similar_movies(movie_name, n_recs, n_user_clusters=10):
    ...
    if len(idx) > 0:  # checks if there is enough data to suggest movies or not
        ...
        for val in indices[0]:    # putting the movies into a list
            ...
            movie_ids += k
        movie_suggestions = pd.DataFrame(data={'movieId':movie_ids, 'titles':suggestions})
        return movie_suggestions
    else:
        print("Data about the movie is not enough")


def recommend_based_on_taste(user_name='ALI', forecast_column='UserRating', threshold=4):
    """
    this function forecasts the rating of a users for the movies they have not rated yet. then selects and then returns
    the movies with ratings more than the threshold. note that the dataframe values are sorted descending by the ratings.
    :param forecast_column: an integer which are number of recommendations that we want from our sysetm.
    :param user_name: a string that is the user id of the person whom we want to forecast their ratings.
    :param threshold: if forecasted rating for a movie is more than this number, the system suggests it to the user.
    ...
    final_dataset = add_user_ratings_column(user_ratings, final_dataset_t, user_name)
    final_dataset.replace({0: pd.NA}, inplace=True)
    train = final_dataset.dropna(axis=0, subset=[forecast_column])
    ...
    forecasted_movie_ids = test_y.index.tolist()
    model = neighbors.KNeighborsRegressor(n_neighbors=7, weights='distance', algorithm='brute', metric='cosine')
    model.fit(train_x, train_y)
    y_hat = model.predict(test_x)
    forecast_results = pd.DataFrame(data={'movieId': forecasted_movie_ids, 'ratings': y_hat})
    suggestions = forecast_results[forecast_results['ratings'] >= threshold]
    suggestions.sort_values(by=['ratings'], ascending=False, inplace=True)
    return suggestions
